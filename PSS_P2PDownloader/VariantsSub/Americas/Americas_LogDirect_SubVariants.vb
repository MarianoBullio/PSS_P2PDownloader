...
Public Class Americas_LogDirect_SubVariants
    ...
    Private Sub Americas_LogDirect_SubVariants_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Try
            SF.Load_ComboBox(List_Variant, CS, "SELECT Variant, Description FROM Var_Americas_LogDirect_Variants Order By Description Asc")
    ...
    Private Sub Fill_DGV(ByVal Variant_Number As Object)
        Try
            ...
            MBS.DataSource = SQL_F.GetDataTable(CS, "Select * From Var_Americas_LogDirect_Var" & Variant_Number)
...
#Region "Functions Tool Bar"
        ...
        Try
            ...
            Select Case Variant_Number
                ...
                Case "01"
                    ...
                    Dim LE As String
                    LE = InputBox("LE Code [It must be Numeric]:", Project_Name, "")

                    If LE = "" Then
                        Exit Sub
                    End If

                    If Not IsNumeric(LE) Then
                        MsgBox("Error: Review 'LE' Value [It must be Numeric]", MsgBoxStyle.Exclamation, Project_Name)
                    Else
                        Dim Insert As String = "Insert Into Var_Americas_LogDirect_Var" & Variant_Number & " Values (" & LE & ")"
                        Dim Exc As String = SQL_F.SQL_Execute_NQ(CS, Insert)
                        If Exc <> Nothing Then
                            If Exc.Contains("Violation") Then
                                MsgBox("This Combination Already exists [LE Code]", MsgBoxStyle.Information, Project_Name)
                            Else
            ...
            If SF.ShowConfirmationMessage() Then
                ...
                Select Case Variant_Number
                    ...
                    Case "01"
                        ...
                        Dim LE As String
                        LE = dgv_Main.CurrentRow.Cells("LE").Value

                        Dim Delete As String = "Delete From Var_Americas_LogDirect_Var" & Variant_Number & " Where(LE=" & LE & ")"
                        Dim Exc As String = SQL_F.SQL_Execute_NQ(CS, Delete)
                        If Exc <> Nothing Then
                            MsgBox(Standart_MessageError, MsgBoxStyle.Exclamation, Project_Name)
                        Else
                            Fill_DGV(List_Variant.SelectedValue)
                            MsgBox("Item has been Deleted [" & LE & "]", MsgBoxStyle.Information, Project_Name)
                        End If
    ...
    Private Sub btn_Copy_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Copy.Click
        Try
            ...
            ''If Variant_Number <> "01" Then
            SQL_F.TableToClipboard(SQL_F.GetDataTable(CS, "Select * From Var_Americas_LogDirect_Var" & Variant_Number))
            ''End If
