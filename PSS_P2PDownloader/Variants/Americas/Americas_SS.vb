...
#Region "Functions Grid 1"
    ...
    Private Sub Fill_Matrix1()
        ...
        Try
            ...
            MBS1.DataSource = SQL_F.GetDataTable(CS, "Select * From View_Americas_SS")
            ...
            For Each C In dgv_Matrix1.Columns
                If C.Name <> "Flag" And C.Name <> "Description" And C.Name <> "Variant_To_Apply" And C.Name <> "Owner" And C.Name <> "Enabled" And C.Name <> "Infosys_ReqToPO" And C.Name <> "Infosys_OpenOrders" And C.Name <> "Pending_Approval" Then
                    C.ReadOnly = True
                End If
            Next
            ...
            'Key Columns
            dgv_Matrix1.Columns("Plant").HeaderCell.Style = Key_Column
            dgv_Matrix1.Columns("SAP_Box").HeaderCell.Style = Key_Column
            dgv_Matrix1.Columns("POrg").HeaderCell.Style = Key_Column
            dgv_Matrix1.Columns("Variant").HeaderCell.Style = Key_Column
            dgv_Matrix1.Columns("Conca_BoxPlantPOrg").HeaderCell.Style = Key_Column
            ...
            dgv_Matrix1.Columns("Description").HeaderCell.Style = Editable_Column
            dgv_Matrix1.Columns("Variant_To_Apply").HeaderCell.Style = Editable_Column
            ...
            'Infosys_ReqToPO
            I = dgv_Matrix1.Columns("Infosys_ReqToPO").DisplayIndex
            dgv_Matrix1.Columns.Remove("Infosys_ReqToPO")
            CC = New DataGridViewComboBoxColumn
            CC.Name = "Infosys_ReqToPO"
            CC.DataPropertyName = "Infosys_ReqToPO"
            CC.HeaderText = "Infosys_ReqToPO"
            CC.DataSource = SQL_F.GetDataTable(CS, "SELECT TNumber, Name FROM Variant_Users Where (Employee) = 0 Order By Name Desc")
            CC.DisplayMember = "Name"
            CC.ValueMember = "TNumber"
            CC.DisplayIndex = I
            dgv_Matrix1.Columns.Add(CC)
            dgv_Matrix1.Columns("Infosys_ReqToPO").AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
            dgv_Matrix1.Columns("Infosys_ReqToPO").HeaderCell.Style = Editable_Column

            'Infosys_OpenOrders
            I = dgv_Matrix1.Columns("Infosys_OpenOrders").DisplayIndex
            dgv_Matrix1.Columns.Remove("Infosys_OpenOrders")
            CC = New DataGridViewComboBoxColumn
            CC.Name = "Infosys_OpenOrders"
            CC.DataPropertyName = "Infosys_OpenOrders"
            CC.HeaderText = "Infosys_OpenOrders"
            CC.DataSource = SQL_F.GetDataTable(CS, "SELECT TNumber, Name FROM Variant_Users Where Employee = 0 Order By Name Desc")
            CC.DisplayMember = "Name"
            CC.ValueMember = "TNumber"
            CC.DisplayIndex = I
            dgv_Matrix1.Columns.Add(CC)
            dgv_Matrix1.Columns("Infosys_OpenOrders").AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
            dgv_Matrix1.Columns("Infosys_OpenOrders").HeaderCell.Style = Editable_Column
    ...
    Private Sub dgv_Matrix1_RowLeave(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Matrix1.RowLeave
        ...
        Try
            If dgv_Matrix1.IsCurrentRowDirty Then
                ...
                Dim R As DataRow
                Dim Plant As String = dgv_Matrix1.CurrentRow.Cells("Plant").Value
                ...
                Dim Variant_ As String = dgv_Matrix1.CurrentRow.Cells("Variant").Value

                Dim DT As DataTable = SQL_F.GetSQLTable(CS, "Var_Americas_SS", "(Plant='" & Plant & "') AND (SAP_Box='" & SAP_Box & "') AND (POrg=" & POrg & ") AND (Variant=" & Variant_ & ")")
                If DT.Rows.Count = 1 Then
                    ...
                    R("Description") = dgv_Matrix1.CurrentRow.Cells("Description").Value
                    R("Variant_To_Apply") = dgv_Matrix1.CurrentRow.Cells("Variant_To_Apply").Value
                    R("Owner") = dgv_Matrix1.CurrentRow.Cells("Owner").Value
                    R("Enabled") = dgv_Matrix1.CurrentRow.Cells("Enabled").Value
                    R("Infosys_ReqToPO") = dgv_Matrix1.CurrentRow.Cells("Infosys_ReqToPO").Value
                    R("Infosys_OpenOrders") = dgv_Matrix1.CurrentRow.Cells("Infosys_OpenOrders").Value
                    ...
                    Dim Update As String = "Update Var_Americas_SS Set " & _
                    "Flag=" & SF.BooleanConvertion(R("Flag")) & ", " & _
                    "Description='" & R("Description") & "'," & _
                    "Variant_To_Apply='" & R("Variant_To_Apply") & "'," & _
                    "Owner='" & R("Owner") & "'," & _
                    "Enabled='" & R("Enabled") & "'," & _
                    "Infosys_ReqToPO='" & R("Infosys_ReqToPO") & "'," & _
                    "Infosys_OpenOrders='" & R("Infosys_OpenOrders") & "'," & _
                    ...
                    "Where (Plant='" & Plant & "') AND (SAP_Box='" & SAP_Box & "') AND (POrg=" & POrg & ") AND (Variant=" & Variant_ & ")"
    ...
    Private Sub Delete_Email_Date(ByRef R As DataRow)
        Try
            Dim Update As String = "Update Var_Americas_SS Set Email_Date=Null Where (Plant='" & R("Plant") & "') AND (SAP_Box='" & R("SAP_Box") & "') AND (POrg=" & R("POrg") & ") AND (Variant=" & R("Variant") & ")"
    ...
    Private Sub Delete_Combination(ByRef R As DataRow)
        Try
            Dim Delete As String = "Delete From Var_Americas_SS Where (Plant='" & R("Plant") & "') AND (SAP_Box='" & R("SAP_Box") & "') AND (POrg=" & R("POrg") & ") AND (Variant=" & R("Variant") & ")"
...
#Region "Functions Tool Bar"
    ...
    Private Sub FlagCheck(ByVal Column As String, ByVal Value As Integer)
        ...
        Try
            ...
            Dim Plant As String = Nothing
            Dim SAP_Box As String = Nothing
            Dim POrg As String = Nothing
            Dim Selec As String = Nothing
            Dim Variant_ As String = Nothing


            If Filter <> "Not (Flag = 'True')" And Filter <> Nothing Then
                Selec = "Select * from " & View & " where " & Filter
            Else
...
#Region "Buttons"
    ...
    Private Sub btn_New_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_New.Click
        ...
        Dim FRM_Small As New Americas_SS_NewRow
        Dim Region As String = ""
        Dim Plant As String = ""
        Dim SAP_Box As String = ""
        Dim POrg As String = ""
        Dim Variant_ As String = ""
        Dim Variant_To_Apply As String = ""
        Dim Owner As String = ""
            ...
            While Not FRM_Small.Result
                If (FRM_Small.ShowDialog() = Windows.Forms.DialogResult.OK) Then
                    If FRM_Small.Result Then
                        Region = FRM_Small.Region_
                        Plant = FRM_Small.Plant_
                        SAP_Box = FRM_Small.SAP_Box_
                        POrg = FRM_Small.POrg_
                        Variant_ = FRM_Small.Variant_
                        Variant_To_Apply = FRM_Small.Variant_To_Apply_
                ...
                Dim Insert As String = "Insert Into Var_Americas_SS(Region,Plant,SAP_Box,POrg,Variant,Variant_To_Apply,Suggested_Owner,Change_Type,Enabled,Owner,Infosys_ReqToPO,Infosys_OpenOrders,Created_Date,Created_By,Updated_Date,Updated_By) Values"
                Insert += "("
                Insert += "'" & Region.Trim & "',"
                Insert += "'" & Plant.Trim & "',"
                Insert += "'" & SAP_Box.Trim & "',"
                Insert += POrg.Trim & ","
                Insert += Variant_.Trim & ","
                Insert += "'" & Variant_To_Apply & "',"
                ...
                If Exc <> Nothing Then
                    If Exc.Contains("Violation") Then
                        MsgBox("This Combination Already exists [Plant,SAP_Box,POrg,Variant]", MsgBoxStyle.Information, Project_Name)
                    Else
    ...
    Private Sub btn_Delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Delete.Click
        Try
            ...
            DT_ToBeDeleted = SQL_F.GetDataTable(CS, "Select * From View_Americas_SS Where (Flag=1)")
            ...
            If DT_ToBeDeleted.Rows.Count > 0 Then
                If SF.ShowConfirmationMessage("Would you like to Delete: [" & DT_ToBeDeleted.Rows.Count & "] Combinations? ", Project_Name) Then
                    For Each R As DataRow In DT_ToBeDeleted.Rows
                        Dim Delete As String = "Update Var_Americas_SS Set Change_Type=3 Where (Plant='" & R("Plant") & "') AND (SAP_Box='" & R("SAP_Box") & "') AND (POrg=" & R("POrg") & ") AND (Variant=" & R("Variant") & ")"
                        Dim Exc As String = SQL_F.SQL_Execute_NQ(CS, Delete)
                        If Exc <> Nothing Then
    ...
    Private Sub btn_Export_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Export.Click
        If SF.ExportVariantToExcel("SS", My.Computer.FileSystem.SpecialDirectories.Desktop & "\Variant - Americas SS (" & Date.Now.Month & "-" & Date.Now.Day & "-" & Date.Now.Year & ").xlsx") Then
